...
{
    // получаем сокет дл€ работы с клиентом
    // это работает с блокировкой (синхронно), т.е. accept() завершитс€ когда подключитс€ клиент,
    // ну или по оооочень большому таймауту выйдем с ошибкой, если клиента долго не будет
    //      второй и третий параметры дл€ переменной и еЄ длины, в которую
    //      вернЄтс€ адрес подключившегос€ клиента, но нам это безразлично, потому NULL и NULL
...
{
    // “окен используетс€ как "session-like" маркер дл€ клиента (AccessToken в Transport).
        ...
        try
        {
            ...
            switch (request.Type)
            {
                ...
                case RequestType::Handshake:
                {
                    try
                    {
                        // ¬ерси€ протокола провер€етс€ на этапе handshake.
...
{
    // Ќа один сокет допускаетс€ параллельна€ работа нескольких потоков (например, UI + updates),
...
std::vector<Responce> Protocol::SendRequestList(const SOCKET* transportSocket, const Request& request, getRemainingFunc getRemaining)
{
    ...
    //  онтракт: remaining == -1 означает "пустой список" (нет элементов).
